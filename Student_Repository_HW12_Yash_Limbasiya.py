...
class UniversityLoader:
            ...
            with open(path, 'r') as file_open:
                ...
                while True:
                    ...
                    if line != '':
                        ...
                        if len(line_divided) == fields_num:
    ...
    def create_uni(self):
        ...
        # Gets all the files with name student.txt
        student_files = glob.glob(folder_path+"/students.txt")
        # Gets all the files with name student.txt
        instructor_files = glob.glob(folder_path+"/instructors.txt")
        # Gets all the files with name student.txt
        grade_files = glob.glob(folder_path+"/grades.txt")
        # Gets all the files with name majors.txt
        major_files = glob.glob(folder_path+"/majors.txt")
        ...
        if(len(student_files) == 1):
            ...
            # Sets the student dict.
            university_obj.student_dict = student_dict
        ...
        if(len(instructor_files) == 1):
            ...
            # sets the instructor dict.
            university_obj.instructor_dict = instructor_dict
        ...
        if len(grade_files) == 1:
            ...
            for student_id, course_name, grade_val, instructor_id in self.read_file_custom(path=grade_files.pop(), fields_num=4, sep='\t', header=self.header):
                ...
                # Checks if the student id is present
                elif university_obj.student_dict.get(student_id) is None:
                    raise KeyError(
                        f"Student with id {student_id} not currently present.")
                # Checks if the instructor id is present.
                elif university_obj.instructor_dict.get(instructor_id) is None:
                    raise KeyError(
    ...
    @staticmethod
    def student_grades_table_db(db_path):
        con = UniversityLoader.create_db_conn(db_path)
        cur = con.cursor()
    ...
    def create_table(self):
        ...
        grades_summary.field_names = [
            'NAME', 'CWID', 'COURSE', 'GRADE', 'INSTRUCTOR']
        grades_summary.add_rows(UniversityLoader.student_grades_table_db(self.db))
...
@app.route("/")
def template_test():
    return render_template('base.html', my_list=UniversityLoader.student_grades_table_db(sys.argv[1:][3]))
